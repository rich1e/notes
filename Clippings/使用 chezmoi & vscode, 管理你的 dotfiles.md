...
source: https://www.shuzhiduo.com/A/kvJ3V17Xzg/
author:
published:
created: 2026-07-25
description:
...
![[assets/Clippings/使用 chezmoi & vscode, 管理你的 dotfiles/IMG-20260725215358776.png]]
...
[diff]
command = "code"
args = ["--wait", "--diff", "{{ .Destination }}", "{{ .Target }}"][merge]
command = "bash"
```

#### 3、敏感数据存储

如果你想用 chezmoi 管理你的密钥（例如: id\_rsa ssh 密钥），同时又想把你的 dotfiles 配置在 GitHub 共享出来，chezmoi 自带了敏感数据存储的方案，可以使用 GPG、AGE 等对配置文件进行加密, 参考: [https://www.chezmoi.io/user-guide/encryption/](https://www.chezmoi.io/user-guide/encryption/)
